...
const window = libsdl.symbols.SDL_CreateWindow(
  Buffer.from(title),
  0x2fff0000,
  0x2fff0000,
  width,
...
if (window == null) throw `WINDOW FAILED TO BE CREATED`;

// Create the renderer
// Renderer is made with vsync
const renderer = libsdl.symbols.SDL_CreateRenderer(window, -1, 0x00000004);

const rsq = libsdl.symbols.SDL_SetHint(
  Buffer.from("SDL_RENDER_SCALE_QUALITY\x00"),
  Buffer.from("2")
);
...
const redCircleNormal = libsdl.symbols.SDL_CreateTextureFromSurface(
  renderer,
  libimage.symbols.IMG_Load(Buffer.from("../Resources/buttons/red/normal.png"))
);
const redCircleHover = libsdl.symbols.SDL_CreateTextureFromSurface(
  renderer,
  libimage.symbols.IMG_Load(Buffer.from("../Resources/buttons/red/hover.png"))
);
const redRect = new Rectangle(32, 18, 12, 12);
const yellowCircleNormal = libsdl.symbols.SDL_CreateTextureFromSurface(
  renderer,
  libimage.symbols.IMG_Load(
    Buffer.from("../Resources/buttons/yellow/normal.png")
  )
);
const yellowCircleHover = libsdl.symbols.SDL_CreateTextureFromSurface(
  renderer,
  libimage.symbols.IMG_Load(Buffer.from("../Resources/buttons/yellow/hover.png"))
);
...
// Create draggie
const draggie = libsdl.symbols.SDL_CreateTextureFromSurface(
  renderer,
  libimage.symbols.IMG_Load(Buffer.from("../Resources/draggie-editor.png"))
);
const draggieRect = new Rectangle(1204, 0, 95, 60);

// Create icon
const icon = libimage.symbols.IMG_Load(
  Buffer.from("resources/icons/draggie.ico")
);
...
// Load font
const font = libttf.symbols.TTF_OpenFont(
  Buffer.from("../Resources/RobotoFlex.ttf"),
  24
);

// Taskbar icons
const folderOpenNormalIcon = libsdl.symbols.SDL_CreateTextureFromSurface(renderer, libimage.symbols.IMG_Load(Buffer.from("../Resources/icons/folderOpen/normal.png")));
const folderOpenRect = new Rectangle(104, 75, 24, 24);

// Create cursors
const arrowCursor = libsdl.symbols.SDL_CreateSystemCursor(0);
const handCursor = libsdl.symbols.SDL_CreateSystemCursor(11);
...
const titleRect = new Rectangle(
  640 - wArr[0] / 2,
  titlebarHeight / 2 - hArr[0] / 2,
  wArr[0],
  hArr[0]
);
const titleTexture = libsdl.symbols.SDL_CreateTextureFromSurface(
  renderer,
  libttf.symbols.TTF_RenderText_Solid(font, Buffer.from(title), white.value)
);
*/
...
while (running) {
  ...
  if (isEvent) {
    switch (eventArray[0]) {
      ...
      case 1024:
        ...
        if (folderOpenRect.boundsCheck(mx, my)) {
            libsdl.symbols.SDL_SetCursor(handCursor);
            break;
        }

        libsdl.symbols.SDL_SetCursor(arrowCursor);
        break;
    }
  }
  ...
  libsdl.symbols.SDL_RenderCopy(
    renderer,
    redRect.boundsCheck(mx, my) ? redCircleHover : redCircleNormal,
    null,
    redRect.pointer
  );
  libsdl.symbols.SDL_RenderCopy(
    renderer,
    yellowRect.boundsCheck(mx, my) ? yellowCircleHover : yellowCircleNormal,
    null,
    yellowRect.pointer
  );
  ...
  // Draw taskbar icons
  libsdl.symbols.SDL_RenderCopy(renderer, folderOpenNormalIcon, null, folderOpenRect.pointer);
